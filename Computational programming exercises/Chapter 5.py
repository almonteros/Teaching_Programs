...
def exercise_find_electric_field():
    ...
    Ex = np.zeros(p.shape, dtype=float)
    Ey = np.zeros(p.shape, dtype=float)
    ...
    plt.quiver(X, Y, Ex/E_mag, Ey/E_mag)
    plt.scatter(sep/2, 0)
    plt.scatter(-sep/2, 0)
    ...
    wx, wy = wp[None, :], wp[:, None]
    xx, xy = xp[None, :], xp[:, None]
    for x_index, x in enumerate(xs):
        for y_index, y in enumerate(ys):
            s = np.sum(wx*wy * dV(xx, xy, x, y))
            V[x_index, y_index] = s
    plt.figure()
